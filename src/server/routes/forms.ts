...
forms.post("/create-post", async (c) => {
    try {
        ...
        // ✅ Create new post
        const postTitle = values.postTitle ?? "FlairForge High Scores";
        ...
        const botComment = await reddit.submitComment({
            ...
            text: formatMessage(
                `This post displays the top **${newData.numberOfUsers}** users with the most ${pointName}s in this subreddit.\n\n` +
                    ...
                    `[Mod info regarding the leaderboard](https://reddit.com/r/${subredditName}/wiki//modinfo)`,
                {},
            ),
        });
...
function formatMessage(
    ...
    const footer = `\n\n---\n\n^(I am a bot - please contact [my creator](https://reddit.com/message/compose?to=/r/FlairForge) with any questions)`;
    if (
        !result
            .trim()
            .endsWith(
